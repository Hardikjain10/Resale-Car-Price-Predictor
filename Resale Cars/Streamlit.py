...
st.title('Resale Car Price Predictor')
st.header('Fill in the details ')

@st.cache(allow_output_mutation=True)
def load_model():
    with open(".\PklFiles\model_xg.pkl",'rb') as f:
        model=pkl.load(f)
    ...
    return model,companies,color_transforms,model_transform,model_names,img_dict
...
model_cars_s=[]
for i in model_cars:
    model_cars_s.append(str(model_company)+' '+i)
    
model_h=st.selectbox('Which model do you own ?',model_cars_s)

mileage=st.slider('Miles Driven:',min_value=500,max_value=100000,step=500,value=20000)
mileage=float(mileage)**(1/3)

cols=tuple(color_transform.keys())
